...
//   // Запит за даними на тиждень (7 днів)
//   useEffect(() => {
...
//       try {
//         // Спочатку отримуємо дати для 7 днів
...
//         dispatch(setWeekWeather({ daily: aggregatedData }));
...
//       <h2>7-Day Weather in {locationName}</h2>
...
//               <p>{formatDate(day)}</p>
//             </div>
//             <p>Temperature: {weekWeather.daily.temperature_2m_max[index]}°C</p>
//             <p>
//               Wind: {weekWeather.daily.wind_speed_10m[index]} km/h, Humidity:{" "}
//               {weekWeather.daily.relative_humidity_2m[index]}%
//             </p>
//           </div>
//         ))}
//       </div>
...
export const OneWeekWeather: React.FC = () => {
  ...
  useEffect(() => {
    const fetchWeekWeather = async (coordinates: Coords | null) => {
      ...
      <div className={styles.forecastContainer}>
        {weekWeather.daily.time.map((day, index) => (
          ...
          >
            <div className={styles.forecastHeader}>
              {getWeatherIcon(
                weekWeather.daily.weather_code[index],
                styles.weatherIcon
              )}
              <p>{formatDate(day)}</p>
            </div>
